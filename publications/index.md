---
layout: default
title: Publications
permalink: /publications/
---

<header class="page-intro publication-intro">
  <a class="page-intro__back" href="{{ '/' | relative_url }}#research"><span aria-hidden="true">←</span> Research</a>
  <p class="page-intro__eyebrow">Research</p>
  <h1>Publications</h1>
  <p>Preprints and publications</p>
  <nav class="research-profiles publication-profiles" aria-label="Research profiles">
    <a class="research-profile" href="https://scholar.google.com/citations?user=tlQGXsEAAAAJ&amp;hl=en" target="_blank" rel="noopener noreferrer">
      <svg aria-hidden="true" viewBox="0 0 24 24"><path d="m3 9 9-5 9 5-9 5zM7 12.5v4.25c2.8 2 7.2 2 10 0V12.5M20 10v6"/></svg>
      Google Scholar
    </a>
    <a class="research-profile" href="https://arxiv.org/search/advanced?advanced=&amp;terms-0-operator=AND&amp;terms-0-term=Ruiqi+Huang&amp;terms-0-field=all&amp;classification-mathematics=y&amp;classification-physics=y&amp;classification-physics_archives=astro-ph&amp;classification-include_cross_list=include&amp;date-filter_by=all_dates&amp;date-year=&amp;date-from_date=&amp;date-to_date=&amp;date-date_type=submitted_date&amp;abstracts=show&amp;size=50&amp;order=-announced_date_first" target="_blank" rel="noopener noreferrer">
      <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M7 4c3.2 2.5 6.8 13.5 10 16M17 4C13.8 6.5 10.2 17.5 7 20"/></svg>
      arXiv
    </a>
    <a class="research-profile" href="https://orcid.org/0009-0005-4708-6846" target="_blank" rel="noopener noreferrer">
      <svg aria-hidden="true" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M9 9v7M9 6.5v.01M12 9h2a3 3 0 0 1 3 3v4M12 16V9"/></svg>
      ORCID
    </a>
  </nav>
</header>

<section class="publications-index" aria-labelledby="publication-list-title">
  <header class="publications-index__heading">
    <h2 id="publication-list-title">Complete list</h2>
    <span>2 works</span>
  </header>
  <ol class="publication-list timeline-list publication-timeline">
    <li class="publication-item timeline-entry">
      <time class="timeline-date" datetime="2026-04">Apr 2026</time>
      <article class="publication-entry">
        <div class="publication-entry__meta">
          <span class="research-status research-status--accepted">Accepted</span>
        </div>
        <h3>Approximating Periodic Orbits with Algebraic Curves and Related Minimal Problems</h3>
        <p class="publication-entry__authors"><strong>Ruiqi Huang</strong> and Anton Leykin</p>
        <p class="publication-entry__venue">Accepted manuscript · 8 pages · 10 figures</p>
        <nav class="publication-entry__links" aria-label="Links for Approximating Periodic Orbits with Algebraic Curves and Related Minimal Problems">
          <a class="research-link research-link--arxiv" href="https://arxiv.org/abs/2604.26332" target="_blank" rel="noopener noreferrer">
            <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M7 4c3.2 2.5 6.8 13.5 10 16M17 4C13.8 6.5 10.2 17.5 7 20"/></svg>
            arXiv
          </a>
          <a class="research-link research-link--github" href="https://github.com/Rick3yHuang/Navigation-Problems-in-CR3BP" target="_blank" rel="noopener noreferrer">
            <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M12 3a9 9 0 0 0-2.84 17.54c.45.08.62-.2.62-.44v-1.59c-2.54.55-3.07-1.08-3.07-1.08-.42-1.06-1.02-1.35-1.02-1.35-.83-.57.06-.56.06-.56.92.06 1.4.94 1.4.94.82 1.4 2.15 1 2.67.76.08-.59.32-1 .58-1.22-2.03-.23-4.16-1.02-4.16-4.46 0-.99.35-1.79.94-2.42-.09-.23-.41-1.16.08-2.39 0 0 .77-.25 2.48.92A8.63 8.63 0 0 1 12 8.35a8.54 8.54 0 0 1 2.26.3c1.72-1.17 2.48-.92 2.48-.92.49 1.23.17 2.16.08 2.39.59.63.94 1.43.94 2.42 0 3.45-2.14 4.23-4.17 4.45.33.28.62.84.62 1.68v2.38c0 .24.16.53.63.44A9 9 0 0 0 12 3z"/></svg>
            GitHub repo
          </a>
          <button class="citation-action citation-action--cite" type="button" aria-expanded="false" aria-controls="citation-periodic">Cite</button>
          <a class="publication-entry__detail" href="{{ '/research/periodic-orbits/' | relative_url }}">Project details <span aria-hidden="true">→</span></a>
        </nav>
        <div class="citation-panel" id="citation-periodic" hidden>
          <div class="citation-panel__header">
            <p>Ruiqi Huang and Anton Leykin. “Approximating Periodic Orbits with Algebraic Curves and Related Minimal Problems.” Accepted manuscript, 2026.</p>
          </div>
          <div class="citation-code">
            <button class="citation-copy" type="button" data-bibtex-id="bibtex-periodic" aria-label="Copy BibTeX" title="Copy BibTeX">
              <svg aria-hidden="true" viewBox="0 0 24 24"><rect x="8" y="8" width="11" height="11" rx="1.5"/><path d="M16 8V6.5A1.5 1.5 0 0 0 14.5 5h-9A1.5 1.5 0 0 0 4 6.5v9A1.5 1.5 0 0 0 5.5 17H8"/></svg>
            </button>
            <pre><code id="bibtex-periodic">@misc{huang2026periodic,
  title        = {Approximating Periodic Orbits with Algebraic Curves and Related Minimal Problems},
  author       = {Huang, Ruiqi and Leykin, Anton},
  year         = {2026},
  eprint       = {2604.26332},
  archivePrefix = {arXiv},
  primaryClass = {math.AG},
  note         = {Accepted manuscript}
}</code></pre>
          </div>
        </div>
      </article>
    </li>
    <li class="publication-item timeline-entry">
      <time class="timeline-date" datetime="2025-11">Nov 2025</time>
      <article class="publication-entry">
        <div class="publication-entry__meta">
          <span class="research-status research-status--published">Published</span>
        </div>
        <h3>Projective Plane Subdivision Method for Initial Orbit Determination</h3>
        <p class="publication-entry__authors"><strong>Ruiqi Huang</strong>, Anton Leykin, and Michela Mancini</p>
        <p class="publication-entry__venue"><em>Computer Algebra in Scientific Computing</em> (CASC 2025) · Lecture Notes in Computer Science, vol. 16235 · Springer · pp. 98–117 · 2026</p>
        <nav class="publication-entry__links" aria-label="Links for Projective Plane Subdivision Method for Initial Orbit Determination">
          <a class="research-link research-link--publication" href="https://doi.org/10.1007/978-3-032-09645-6_6" target="_blank" rel="noopener noreferrer">
            <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M6 3h9l3 3v15H6zM15 3v4h3M9 11h6M9 15h6"/></svg>
            DOI
          </a>
          <a class="research-link research-link--arxiv" href="https://arxiv.org/abs/2509.14397" target="_blank" rel="noopener noreferrer">
            <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M7 4c3.2 2.5 6.8 13.5 10 16M17 4C13.8 6.5 10.2 17.5 7 20"/></svg>
            arXiv
          </a>
          <button class="citation-action citation-action--cite" type="button" aria-expanded="false" aria-controls="citation-iod">Cite</button>
          <a class="publication-entry__detail" href="{{ '/research/initial-orbit-determination/' | relative_url }}">Project details <span aria-hidden="true">→</span></a>
        </nav>
        <div class="citation-panel" id="citation-iod" hidden>
          <div class="citation-panel__header">
            <p>Ruiqi Huang, Anton Leykin, and Michela Mancini. “Projective Plane Subdivision Method for Initial Orbit Determination.” In <em>Computer Algebra in Scientific Computing</em>, pp. 98–117. Springer, 2026.</p>
          </div>
          <div class="citation-code">
            <button class="citation-copy" type="button" data-bibtex-id="bibtex-iod" aria-label="Copy BibTeX" title="Copy BibTeX">
              <svg aria-hidden="true" viewBox="0 0 24 24"><rect x="8" y="8" width="11" height="11" rx="1.5"/><path d="M16 8V6.5A1.5 1.5 0 0 0 14.5 5h-9A1.5 1.5 0 0 0 4 6.5v9A1.5 1.5 0 0 0 5.5 17H8"/></svg>
            </button>
            <pre><code id="bibtex-iod">@inproceedings{huang2026projective,
  title     = {Projective Plane Subdivision Method for Initial Orbit Determination},
  author    = {Huang, Ruiqi and Leykin, Anton and Mancini, Michela},
  booktitle = {Computer Algebra in Scientific Computing},
  series    = {Lecture Notes in Computer Science},
  volume    = {16235},
  pages     = {98--117},
  publisher = {Springer},
  year      = {2026},
  doi       = {10.1007/978-3-032-09645-6_6}
}</code></pre>
          </div>
        </div>
      </article>
    </li>
  </ol>
</section>
